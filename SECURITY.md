...
| File | What it is |
|---|---|
| `pso-protocol-X.Y.Z.crate` | The byte-identical .crate uploaded to crates.io. |
| `pso-protocol-X.Y.Z.crate.sig` | cosign blob signature (raw, base64). |
| `pso-protocol-X.Y.Z.crate.pem` | Fulcio-issued ephemeral signing cert (PEM). |
...
The Solidity side has no separate artifact (consumed via `forge install` from git tags), so the signing surface is the same as the other Rust-only psonet repos.
...
- A compromise of `github.com/psonet/pso-protocol` itself (an attacker with push access to `main` can edit the workflow to remove or weaken signing).
...
- Tampering with the Solidity side. The Solidity contracts are consumed via `forge install` from git tags; signature coverage there would require a separate signing flow over the source-tree state and is out of scope for T-0020.
...
```sh
REPO=psonet/pso-protocol
TAG=v0.2.3  # or any release ≥ the cutoff
...
For security issues in `pso-protocol` itself (not the signing pipeline), open a [private security advisory](https://github.com/psonet/pso-protocol/security/advisories/new) on GitHub. Do not file a public issue.
